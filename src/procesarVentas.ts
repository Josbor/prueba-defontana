  ...
  async function procesarVentas(): Promise<void> {
    ...
    const ventasUnicas = ventas.filter((v, index, self) => 
      index === self.findIndex((t) => t.ID_Venta === v.ID_Venta)
  );
  const totalVentasUnicas = ventasUnicas.reduce((sum, v) => sum + v.Total, 0);

  const totalVentas = totalVentasUnicas;
  const cantidadVentas =ventasUnicas.length;

    // Venta más alta
    const ventaMasAlta = ventasUnicas.reduce((max, v) => (v.Total > max.Total ? v : max), ventas[0]);
    ...
    console.log(`🔹 Producto más vendido por local:`);
    const productoMasVendidoPorLocalFormatted = productoMasVendidoPorLocal.map((item) => {
      const [producto, cantidad] =item.producto
      return {
          local: item.local,
          producto: producto,
          cantidad:cantidad
  ...
  console.table(productoMasVendidoPorLocalFormatted)
